...
type GlobalNetworkSetInformer interface {
	...
	Lister() internalversion.GlobalNetworkSetLister
}
...
func NewGlobalNetworkSetInformer(client internalclientset.Interface, resyncPeriod time.Duration, indexers cache.Indexers) cache.SharedIndexInformer {
...
// Always prefer using an informer factory to get a shared informer instead of getting an independent
// one. This reduces memory footprint and number of connections to the server.
func NewFilteredGlobalNetworkSetInformer(client internalclientset.Interface, resyncPeriod time.Duration, indexers cache.Indexers, tweakListOptions internalinterfaces.TweakListOptionsFunc) cache.SharedIndexInformer {
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.Projectcalico().GlobalNetworkSets().List(context.TODO(), options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.Projectcalico().GlobalNetworkSets().Watch(context.TODO(), options)
			},
		},
		&projectcalico.GlobalNetworkSet{},
		resyncPeriod,
		indexers,
	)
}

func (f *globalNetworkSetInformer) defaultInformer(client internalclientset.Interface, resyncPeriod time.Duration) cache.SharedIndexInformer {
...
func (f *globalNetworkSetInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&projectcalico.GlobalNetworkSet{}, f.defaultInformer)
}

func (f *globalNetworkSetInformer) Lister() internalversion.GlobalNetworkSetLister {
	return internalversion.NewGlobalNetworkSetLister(f.Informer().GetIndexer())
}
